      ...
      <div className="grid-dashboard">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '0.5rem' }}>
              {orders.map(ord => (
                <div
                  ...
                  style={{
                    padding: '1.25rem',
                    background: selectedOrderId === ord.id ? '#F0FDF4' : 'var(--bg-main)',
                    borderRadius: 'var(--radius-sm)',
                    border: `1px solid ${selectedOrderId === ord.id ? '#B8E1C4' : 'var(--border-color)'}`,
                    cursor: 'pointer',
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem' }}>
                    <div>
                      ...
                      <h4 style={{ fontSize: '1.05rem', fontWeight: 600, marginTop: '0.4rem', color: 'var(--text-main)' }}>{ord.item}</h4>
        ...
        <div className="col-span-6" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <Card title={`Order Details — #${selectedOrder.id}`} subtitle={`Status: ${selectedOrder.status.replace('_', ' ')}`}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', padding: '1rem', background: 'var(--bg-main)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)', marginBottom: '1.5rem' }}>
              <div>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600 }}>BUYER & DELIVERY ADDRESS</span>
                <h5 style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-main)', marginTop: '0.2rem' }}>{selectedOrder.buyerName}</h5>
              ...
              <div>
                ...
                <h5 style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-main)', marginTop: '0.2rem' }}>{selectedOrder.vendor}</h5>
                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Verified Regional Distribution Hub</p>
              </div>
            </div>

            {/* Tracking Progress Bar matching Stitch */}
            <h4 style={{ fontSize: '0.95rem', fontWeight: 600, color: 'var(--text-main)', marginBottom: '1rem' }}>Fulfillment Timeline</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', paddingLeft: '0.5rem', borderLeft: '2px solid var(--primary)' }}>
              <div style={{ paddingLeft: '0.75rem' }}>
                <h5 style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-main)' }}>✔ Order Placed & Payment Confirmed</h5>
                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Aug 24, 2026 • 10:15 AM</p>
              </div>
              <div style={{ paddingLeft: '0.75rem' }}>
                <h5 style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-main)' }}>✔ Dispatched from Regional Warehouse</h5>
                <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Aug 25, 2026 • 06:30 AM</p>
              </div>
              <div style={{ paddingLeft: '0.75rem' }}>
                <h5 style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--primary)' }}>🚚 Out for Doorstep Delivery</h5>
                <p style={{ fontSize: '0.75rem', color: 'var(--primary)', fontWeight: 500 }}>{selectedOrder.estimatedDelivery}</p>
              </div>
            </div>
            ...
            <div style={{ marginTop: '1.5rem', padding: '1rem', background: '#FFFDF5', border: '1px solid #FCD34D', borderRadius: 'var(--radius-sm)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <h5 style={{ fontSize: '0.9rem', fontWeight: 600, color: '#92400E' }}>Delivery Verification Code (OTP)</h5>
                  <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>Share this code with the delivery partner upon arrival.</p>
                </div>
                <div style={{ padding: '0.5rem 1rem', background: '#FFFFFF', border: '2px dashed #F59E0B', borderRadius: '6px', fontSize: '1.4rem', fontWeight: 800, color: 'var(--text-main)', letterSpacing: '2px' }}>
                  {selectedOrder.otp}
                </div>
              </div>
            </div>
